/**
 * 这段代码涉及到几个主要模块的功能，包括：
 * 文档索引与添加： 通过爬取网页生成文章并分割为块，接着生成嵌入（embeddings），然后将嵌入的数据存储到 SQLite 和 LanceDB 两个数据库中。
 * 配置与同步： DocsService 会根据配置文件加载文档并定期同步，如果配置文件中的文档有变化，就会对新增或删除的文档进行相应的操作。
 * 嵌入式提供器： 使用不同的嵌入式提供器生成文档的向量嵌入（如TransformersJsEmbeddingsProvider）。根据不同的情况（如是否为 JetBrains）决定使用哪种嵌入式模型。
 * 数据存储： 文档索引信息（如标题、URL、图标等）会存储到 SQLite 数据库中，而文档的具体内容和嵌入会存储到 LanceDB 中，LanceDB 是一个支持向量搜索的数据库。
 * 文档删除和更新： 如果用户手动删除文档或修改配置，服务会清除旧的索引，并重新爬取与索引文档。
 * 
 * 核心逻辑
 * 文档索引流程：通过爬取网页，获取每个网页的内容，然后生成相应的嵌入（Embeddings）。这些嵌入数据被分割成块（Chunks），存储在数据库中。
 * 数据库操作：LanceDB 用于存储向量数据（如文档的嵌入向量），并通过向量搜索来检索相关文档。而 SQLite 用于存储文档的元数据，如标题、URL、图标等信息。
 * 配置更新和同步：当配置文件发生变化时（例如，更新了要索引的文档列表），系统会重新爬取新增的文档并将其索引到数据库中。同时，也会删除不再需要的文档。
 */
import {
  Chunk,
  ContinueConfig,
